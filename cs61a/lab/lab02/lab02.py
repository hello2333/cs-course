...
def cycle(f1, f2, f3):
    ...
    def compound_call(f,g):
        def compound_call_by_x(x):
            return f(g(x))
        return compound_call_by_x
    
    def cycle_call(n):
        i = 1
        curr_call = call_for(0)
        while i <= n:
            curr_call = compound_call(call_for(i), curr_call)
            i += 1
        return curr_call
    return cycle_call
