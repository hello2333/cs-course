...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attemps = []
    def withdraw_with_pwd(amount, input):
        nonlocal attemps, balance
        if len(attemps) >= 3:
            return "Too many incorrect attempts. Attempts: " + str(attemps)
        if input != password:
            attemps.append(input)
            return 'Incorrect password'
        if balance < amount:
            return 'Insufficient funds'
        balance -= amount
        return balance
    return withdraw_with_pwd
